...
func (build *Builder) Build() *PxDb {
	...
	case Driver_LibSQL:
		...
		break;
	case Driver_DuckDB: dsn = Fmt.Sprintf(string(DSN_DuckDB), pathdb+filedb); break;
...
func (pxtb *PxTb) Field(field_name string, field_type string) *PxTb {
	...
	case "TIMESTAMP":
		...
		case Driver_LibSQL: field_type = `INTEGER NOT NULL`;  break;
		case Driver_DuckDB: field_type = `TIMESTAMP`;         break;
